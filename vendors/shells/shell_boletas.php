...
class ShellBoletasShell extends Shell
{
    var $Controller = null;
        ...
        if(is_readable($ruta)) {
            ...
            if (($handle = fopen($ruta, 'r')) !== FALSE) {
                while (($datos = fgetcsv($handle, 0, $separador)) !== FALSE) {
                    ...
                    if (!isset($registro['Numero Boleta'])) {
                        $stats['parametro_invalido']++;
                    ...
                    if ($compra['Compra']['boleta'] == $registro['Numero Boleta']) {
                        $stats['omitidos']++;
    ...
    function enviar_email_interno($datos)
    {
        ...
        $email->to = $personal;
        //$this->Email->bcc	= $copias;
        $email->subject = 'Correo con status de boletas';
        // $email->from = 'Skechers <' . $this->email_skechers[2] . '>';
        // $this->Email->replyTo = $this->email_skechers[2];
        $email->sendAs = 'html';
