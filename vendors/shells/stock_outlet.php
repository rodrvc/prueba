...
class  StockOutletShell extends Shell {
	var $Controller = null;
	...
	//	App::import('Vendor', 'autoload', array('file' => 'google'.DS.'autoload.php'));
		...
		{
			$excluir [$bloqueo['Bloqueo']['sku']]= array('producto_id' => $bloqueo['Bloqueo']['producto_id'],'cantidad' => $bloqueo['Bloqueo']['cantidad'],'talla' => $bloqueo['Bloqueo']['talla'], 'sku' => $bloqueo['Bloqueo']['sku'], 'created' => $bloqueo['Bloqueo']['created']);
			$skus []= $bloqueo['Bloqueo']['sku'];
		}
		$sql = "select created from sitio_stocks  order by created desc limit 1";
		$respuesta = $ProductoOBJ->query($sql);
		$hoy = $respuesta[0]['sitio_stocks']['created'];
		$sql = "Select * from sitio_quiebres";
		$quiebres = $ProductoOBJ->query($sql);
		//$hoy = '2020-06-16 18:00:00';
		$sql = "select count(*) as cantidad, producto_id, talla, pc.sku  from sitio_compras c, sitio_productos p, sitio_productos_compras pc where p.id = pc.producto_id and  pc.compra_id = c.id  and c.estado = 1 and c.created > '".$hoy."' and pc.producto_id not in (13327,13326)  group by producto_id, talla HAVING COUNT(*) > 5";
		$respuesta = $ProductoOBJ->query($sql);
		$block =array();
		$productos = array();
		...
		$sql = "select count(*) as cantidad, producto_id, talla from sitio_compras c, sitio_productos p, sitio_productos_compras pc where p.id = pc.producto_id and p.categoria_id in (58,59,60) and  pc.compra_id = c.id  and c.estado = 1 and c.created > '".$hoy."' group by producto_id, talla HAVING COUNT(*) > 2";
		$respuesta = $ProductoOBJ->query($sql);
		$block =array();
		$productos = array();
		foreach ($respuesta as $stock)
