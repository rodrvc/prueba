...
class ShellTangoShell extends Shell
{
    var $Controller = null;
        ...
        if(is_readable($ruta)) {
            ...
            if (($handle = fopen($ruta, 'r')) !== FALSE) {
                while (($datos = fgetcsv($handle, 0, $separador)) !== FALSE) {
                    ...
                    if (!isset($registro['Numero despacho TGO'])) {
                        $stats['parametro_invalido']++;
                    ...
                    if (!$registro['Numero despacho TGO']) {
                        $stats['parametro_invalido']++;
                   ...
                   if (!$compra = $Compras->find('first', $options)) {
                    ...
                    if ($compra['Compra']['cod_despacho'] == $registro['N° Orden Skechers']) {
                        $stats['omitidos']++;
                    ...
                    $save = array(
                        'id' => $compra['Compra']['id'],
                        'cod_despacho' => trim($registro['N° Orden Skechers']),
    ...
    function enviar_email($compra){
        ...
        $template = 'despacho';
            ...
            $email->subject = '[Skechers - Tienda en linea] Solicitud de devolución #' . $compra['Compra']['id'];
    ...
    function enviar_email_interno($datos)
    {
        ...
        $email->to = $personal;
        //$this->Email->bcc	= $copias;
        $email->subject = 'Correo con status de tango';
        // $email->from = 'Skechers <' . $this->email_skechers[2] . '>';
        // $this->Email->replyTo = $this->email_skechers[2];
        $email->sendAs = 'html';
