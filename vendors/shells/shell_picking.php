...
class ShellPickingShell extends Shell
{
    var $Controller = null;
        ...
        if(is_readable($ruta)) {
            ...
            if (($handle = fopen($ruta, 'r')) !== FALSE) {
                while (($datos = fgetcsv($handle, 0, $separador)) !== FALSE) {
                    ...
                    if (!isset($registro['Picking number'])) {
                        $stats['parametro_invalido']++;
                    ...
                    if ($compra['Compra']['picking_number'] == $registro['Picking number']) {
                        $stats['omitidos']++;
                    ...
                    $save = array(
                        'id' => $compra['Compra']['id'],
                        'picking_number' => $registro['Picking number']
                    );
    ...
    function enviar_email_interno($datos)
    {
        ...
        $email->to = $personal;
        //$this->Email->bcc	= $copias;
        $email->subject = 'Correo con status de picking';
        // $email->from = 'Skechers <' . $this->email_skechers[2] . '>';
        // $this->Email->replyTo = $this->email_skechers[2];
        $email->sendAs = 'html';
