...
//			$resultado['bkp_name'] = "bkp_".strtotime(date('Y-m-d H:i:s')).'.csv';
	...
	function ajax_descuento()
	{
		...
		if (! $this->params['form']['id'])
		{
			$respuesta = array(
				'estado' => 'ERROR',
				'mensaje' => '<b>Código Inválido</b> revisa si está bien escrito.'// id del producto invalido
			);
			die(json_encode($respuesta));
		}
		if (! $this->params['form']['codigo'])
		{
			$respuesta = array(
				'estado' => 'ERROR',
				'mensaje' => '<b>Código Inválido</b> revisa si está bien escrito.'// codigo invalido
			);
			die(json_encode($respuesta));
		}
