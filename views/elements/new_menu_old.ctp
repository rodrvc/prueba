...
$new_menu = array(
	...
	6 => array(
		...
		'url'			=> 'https://www.skechers.cl/sale',
		'tallas'		=> false,
		'colores'		=> false,
		'genero'		=> true
	),
...
// validar evento
if ($estado_sitio && isset($_evento['menu']))
{
	$new_menu = $_evento['menu'];
}
...
if ($this->params['controller'] == 'productos' && $this->params['action'] == 'performance' && isset($new_menu[5]))
{
	if ($new_menu[5]['class'])
		$new_menu[5]['class'].=' ';
	$new_menu[5]['class'].='active';
}
if ($this->params['controller'] == 'productos' && $this->params['action'] == 'escolar' && isset($new_menu[7]))
{
	if ($new_menu[7]['class'])
		$new_menu[7]['class'].=' ';
	$new_menu[7]['class'].='active';
}
if ($this->params['controller'] == 'productos' && $this->params['action'] == 'cybermonday2016' && isset($new_menu[6]))
{
	if ($new_menu[6]['class'])
		$new_menu[6]['class'].=' ';
	$new_menu[6]['class'].='active';
}
if ($this->params['controller'] == 'tiendas' && $this->params['action'] == 'index' && isset($new_menu[9]))
{
	if ($new_menu[9]['class'])
		$new_menu[9]['class'].=' ';
...
#navHeader > ul.navbar-skechers > li.cybermonday2016.active > a, #navHeader > ul.navbar-skechers > li.cybermonday2016 >  a:hover  {
...
<nav id="navHeader" class="navbar navbar-default yamm hidden-xs hidden-sm" role="navigation">
	<ul class="nav navbar-nav navbar-skechers">
		<? foreach ($new_menu as $tab_menu) : ?>
			...
			<? if ($tab_menu['activo']) : ?>
				<? if (isset($tab_menu['evento']) && $tab_menu['evento']) : ?>
				<li class="<?= $tab_menu['class']; ?>">
					<a style="padding-left: 0px;   padding-top: 0px;    padding-right: 0px;    margin-right: -5;    padding-bottom: 0px;    border-bottom-width: 0px; margin-right: 0px;"" href="<?= (isset($tab_menu['url']) && $tab_menu['url'] ? $tab_menu['url']:'#'); ?>">
						<img src="<?= $this->Shapeups->imagen('cybermonday2016/boton-azul.png'); ?>" alt="">
					</a>
				</li>
				<? else : ?>
					...
					<li class="<?= $tab_menu['class']; ?>">
						<?= $linea; ?>
						<a data-toggle="dropdown" class="dropdown-toggle" style="margin-right: 9px;" href="#" onclick="document.location.href = '<?= $tab_menu['url']; ?>'"><?= $tab_menu['titulo']; ?></a>
				...
				<? elseif ($tab_menu['titulo'] =='COLLECTIONS') :?>

					<li class="<?= $tab_menu['class']; ?>">
						<?= $linea; ?>
						<a data-toggle="dropdown" class="dropdown-toggle" style="margin-right: 9px" href="#" onclick="document.location.href = '<?= $tab_menu['url']; ?>'"><?= $tab_menu['titulo']; ?></a>
						<ul class="dropdown-menu dropdown-menutop" style ="width:250px;left:38% !important">
							<li class="grid-demo">
								<div class="contenido-menutop">
										<div class="col-md-12">
											<h5 style="font-weight: bold;">COLLECTIONS:</h5>
											<div class="row">
												<? foreach ($menu_colecciones as  $coleccion) : ?>
													<? if($coleccion): ?>
														<div class="col-xs-12">
															<a href="<?= $this->Html->url(array('controller' => 'productos', 'action' => 'grupos', $coleccion['Estilo']['alias']));?>" class="list-color"><?= $coleccion['Estilo']['nombre']; ?></a>
														</div>
													<? endif; ?>
					...
					<li class="<?= $tab_menu['class']; ?>">
						<?= $linea; ?>
						<a style="margin-right: 9px;" href="<?= $tab_menu['url']; ?>"><?= $tab_menu['titulo']; ?></a>
					</li>
					<? endif; ?>
				<? endif; ?>
				<? $linea = '<span class="lines"></span>'; ?>
			<? endif; ?>
		<? endforeach; ?>
	</ul>
	<?= $this->Form->create('Producto',
		...
		<div class="input-group relative" style="width: 200px;">
			...
			<!-- AUTOCOMPLETE -->
			<ul class="autocomplete list-unstyled" style="display:none;z-index:99999" rel="autocomplete"></ul>
			<!-- /////////// -->
		</div>
	<?= $this->Form->end(); ?>
</nav>

<nav id="navMobile" class="navbar navbar-default hidden-lg hidden-md" role="navigation">
			...
			<li class="dropdown">
				<? foreach ($new_menu as $tab_menu) : ?>
					<? if ($tab_menu['activo']) : ?>
						<? if (isset($tab_menu['evento']) && $tab_menu['evento']) : ?>
						<li class="<?= $tab_menu['class']; ?>">
							<a href="<?= ($tab_menu['url']) ? $tab_menu['url']:'#'; ?>">
								<img src="<?= $this->Html->url('/img/cybermonday2016/boton-azul.png'); ?>" alt="">
							</a>
						</li>
						<? else : ?>
							<? if ($tab_menu['tallas'] || $tab_menu['colores']) : ?>
													...
													<div class="row nomargin submenu-mobil">
														<h3 class="text-center" style="font-weight: bold;"></h3>
															...
															<? if($coleccion): ?>
																...
																<div class="col-xs-6">
																<a href="<?= $this->Html->url(array('controller' => 'productos', 'action' => 'grupos', $coleccion['Estilo']['alias']));?>" class="list-color" style="color: #fff;"><?= $coleccion['Estilo']['nombre']; ?></a>
																</div>
															<? endif; ?>
														<? endforeach; ?>
													</div>
	...
	<?= $this->Form->create('Producto',
		...
		<div class="input-group relative" >
			...
			<span class="input-group-btn">
				<button class="btn btn-primary btn-buscar" type="button" onclick="$(this).parents('form').submit();">Buscar</button>
			</span>
			<!-- AUTOCOMPLETE -->
			<ul width="100%" max-height="400px"; class="autocomplete list-unstyled" style="display:none;z-index:99999" rel="autocomplete2"></ul>
			<!-- /////////// -->
		</div>
	<?= $this->Form->end(); ?>
</nav>
