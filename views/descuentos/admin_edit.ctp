<div class="col02">
	...
	<ul class="edit">
		...
		<li class="radio-descuento">
			<span class="texto">Tipo de Descuento</span>
			<?= $this->Form->radio('tipo', $opciones_radio, $attr_radio); ?>
	...
	<ul class="edit">
		...
		<li>
			...
			<?= $this->Form->checkbox('Categoria.' .$index. '.categoria_id', array('class' => 'check-categoria')); ?>
		</li>
		<? endforeach; ?>
	</ul>
	<!--showroom-->
	...
	<ul class="edit">
		...
		{
			$options = array(
				'class' => 'check-showroom',
				'value' => $showroom
			);
			if (isset($this->data['Descuento']['showroom']))
			{
				if (in_array($showroom,explode(',',$this->data['Descuento']['showroom'])))
					$options['checked'] = 'checked';
			}
			echo '<li><label class="texto">'.$showroom.'</label>'.$this->Form->checkbox('Showroom.' .$index. '.valor',$options).'</li>';
			$index++;
		}
		?>
	</ul>
